...
function makeScatterPlot(data) {
  ...
  context.fillText("TOEFL Scores", 250, 490);
  context.save();
  context.translate(-240, 250);
  context.rotate(-Math.PI/2);
  context.textAlign = "center";
  context.fillText("Chance of Admit", 0, 250);
  context.restore();
  // context.fillText("Chance of Admit", 10, 250)
...
function drawAxesTicks(axesLimits) {
  ...
  for (let x = 80; x < 500; x += 25) {
    ...
    line(x, 440, x, 460);
    context.fillText(xMark, x - 2, 470);
    // increment counter
    xMark += 2;
  }
...
function toCanvasPoint(point) {
  const xCanvas = (point["TOEFL Score"] - 92)*12 + 80;
  // const xCanvas = (point["TOEFL Score"] - 285)*6 + 50; // scale the x point
  // const yCanvas = (point["Chance of Admit"]); // scale the y point
...
function drawRegressionLine() {
  let startPoint = regressionLine(92); // Use min x value
  let endPoint = regressionLine(120); // Use max x value
